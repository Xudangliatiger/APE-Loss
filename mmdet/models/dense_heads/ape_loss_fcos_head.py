...
class APELossFCOSHead(AnchorHead):
    """
    ...
    def __init__(self,
                 ...
                 #     use_sigmoid=True,
                 #     loss_weight=1.0),
                 loss_cls = dict(type='FastAPLoss',lamb=4,th=0.25,topk=100000),
                 **kwargs):
        self.stacked_convs = stacked_convs
        self.conv_cfg = conv_cfg
        self.norm_cfg = norm_cfg
        super(APELossFCOSHead, self).__init__(num_classes, in_channels, **kwargs)
        self.sampling = False
        self.ranking_loss =  build_loss(loss_cls)
        if self.train_cfg:
    ...
    def iou(self, pred, target, eps=1e-7):
        """
    ...
    #             pos_anchors, pos_bbox_targets)
    #         ious = self.iou(pos_decode_bbox_pred, pos_decode_bbox_targets).clone().detach()
    #         # regression loss
    ...
    #
    #         loss_cls, loss_monitoring, rank, order = self.fap_loss(flat_preds, flat_labels, loss_bbox, ious)
    #     else:
    #         loss_bbox = bbox_pred.sum() * 0
    #         loss_cls = cls_score.sum() * 0
    #         ious = torch.tensor(0).cuda()
    #         # loss_centerness = centerness.sum() * 0
    #         # centerness_targets = torch.tensor(0).cuda()
    #
    #     return loss_cls, loss_bbox, ious.sum() #, loss_centerness, centerness_targets.sum()
    # , 'centernesses'
    ...
    def loss(self,
        ...
        all_bbox_preds = []
        all_bbox_anchors = []
        for labels, label_weights, cls_score, bbox_targets, bbox_weights, bbox_pred,anchor_ in zip(labels_list,
                label_weights_list,cls_scores,bbox_targets_list,bbox_weights_list,bbox_preds,anchor_list):
            ...
            all_bbox_anchors.append(anchor_.reshape(-1, 4))
        ...
        if pos_idx.sum() > 0:
            ...
            pos_anchor = torch.cat(all_bbox_anchors)[pos_idx]
            ...
            pos_decode_bbox_pred = self.bbox_coder.decode(
                pos_anchor, pos_pred)
            pos_decode_bbox_targets = self.bbox_coder.decode(
                pos_anchor, pos_target)

            cls_scores = torch.cat(all_cls_scores)
            ind_2 = cls_labels[pos_idx]
            pos_weights = cls_scores[pos_idx,ind_2].sigmoid().detach()
            ...
            ious = self.iou(pos_decode_bbox_pred,pos_decode_bbox_targets).clone().detach()
            losses_cls = self.ranking_loss(flat_preds, flat_labels, ious)
