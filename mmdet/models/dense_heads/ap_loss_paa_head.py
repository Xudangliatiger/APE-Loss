...
class APLossPAAHead(AnchorHead):
    ...
    def __init__(self,
        ...
        super(APLossPAAHead, self).__init__(*args, **kwargs)
 ...
 #
    ...
    def loss(self,
        ...
        if num_pos:
            ...
            flat_preds = cls_scores.view(-1)#[pos_inds_flatten]
            #flat_labels = labels#[pos_inds_flatten]
            losses_cls = self.ap_loss.apply(flat_preds, flat_labels, iou_target)
    ...
    def get_pos_loss(self, anchors, cls_score, bbox_pred, label, label_weight,
        ...
        ranking_score = torch.zeros_like(iou)
        for gt_ind in range(num_gt):
            ...
            temp_i = iou[gt_mask]
            # _, index1 = temp_i.sort(0)
            # _, index2 = index1.sort(0)
            # p_sorted, _ = temp_p.sort(0)
            # i_ot = p_sorted[index2]
            # temp_i = i_ot
            ...
            ranking_score[gt_mask] =(1-temp_p)+(1-temp_i)

            #torch.sqrt_((1-temp_p)**2+(1-temp_i)**2)

            gt_num_monit = len((ranking_score[gt_mask])[ranking_score[gt_mask]<=0.7])
            if gt_num_monit == 0:
                ranking_score[gt_mask] = 0
            ...
            # for i in ranking_score[gt_mask]:
            #     if i != i:
            #         a = 0
    ...
    def paa_reassign(self, pos_losses, label, label_weight, bbox_weight,
        ...
        for gt_ind in range(num_gt):
            ...
            if len(pos_inds_gmm) < 2:
                continue
            pos_mask =  pos_loss_gmm <= 0.7
            pos_inds_temp = pos_inds_gmm[pos_mask]
            #print(len(pos_inds_temp))
        ...
        num_pos = len(pos_inds[pos_losses<=0.7])
        #print(num_pos)
