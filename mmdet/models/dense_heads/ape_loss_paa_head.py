...
from mmdet.core import (multi_apply, multiclass_nms, anchor_inside_flags, build_assigner, build_sampler,
                        force_fp32, images_to_levels, multi_apply,vectorize_labels,
...
class APELossPAAHead(AnchorHead):
    ...
    def __init__(self,
                 *args,
                 ...
                 loss_cls=dict(type='FastAPLoss', lamb=4, th=0.5, topk=100000),
                 **kwargs):
        ...
        super(APELossPAAHead, self).__init__(*args, **kwargs)
        self.ape_loss = build_loss(loss_cls)
 ...
 #
    ...
    def loss(self,
        ...
        pos_losses_list, = multi_apply(self.get_pos_loss, anchor_list,
                                       ...
                                       bboxes_weight, pos_inds, pos_gt_index)
        ...
        if num_pos:
            ...
            flat_preds = cls_scores.view(-1)

            losses_ranking = self.ape_loss(flat_preds, flat_labels, iou_target)
            ...
            losses_ranking = cls_scores.sum() * 0
            losses_bbox = bbox_preds.sum() * 0

        return dict(
            loss_cls=losses_ranking, loss_bbox=losses_bbox)#, loss_iou=losses_iou)


    def get_pos_loss(self, anchors, cls_score, bbox_pred, label, label_weight,
                     bbox_target, bbox_weight, pos_inds, instance_key):
        ...
        Args:
            ...
            instance_key: Index of each labeled instance, we use all corresponding samples for clustering.
        ...
        # p is the ranking scores
        p = cls_score[pos_inds,pos_label].sigmoid_()
        num_gt = instance_key.max()+1
        iou = bbox_overlaps(
                pos_bbox_pred.detach(), pos_bbox_target, is_aligned=True)

        clustering_input = torch.zeros_like(iou)
        for gt_ind in range(num_gt):
            gt_mask = instance_key == gt_ind

            # temp_p is the ranking scores
            temp_p = p[gt_mask]
            # normalization
            if len(temp_p)>2:
                pass
            ...
            # temp_i is the localization scores
            temp_i = iou[gt_mask]
            if temp_i.max() ==0:
                continue
            # normalization
            ...
            # GMM clustering input equals to the sum of P and I.
            # (Due to the fact that PAA uses losses as input,
            #  thus small input indicates positive sample.
            #  Therefore, we use 1-scores to avoid changes of other PAA codes)
            clustering_input[gt_mask] = (1-temp_p)+(1-temp_i)
    ...
    #     return super(AnchorHead, self)._get_targets_single(
    #         flat_anchors,
    #         valid_flags,
    #         gt_bboxes,
    #         gt_bboxes_ignore,
    ...
    def _get_bboxes(self,
        ...
        for cls_score, bbox_pred, anchors in zip(
            ...
            if nms_pre > 0 and scores.shape[1] > nms_pre:
                max_scores, _ = (scores).sqrt().max(-1)
