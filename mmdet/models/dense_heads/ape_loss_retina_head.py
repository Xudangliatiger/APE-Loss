...
class APELossRetinaHead(AnchorHead):
    ...
    def __init__(self,
                 ...
                 loss_cls=dict(type='FastAPLoss', lamb=4, th=0.5, topk=1000000),
                 **kwargs):
        self.stacked_convs = stacked_convs
        self.conv_cfg = conv_cfg
        self.norm_cfg = norm_cfg
        super(APELossRetinaHead, self).__init__(
            num_classes,
            in_channels,
...
#        self.period = 783*2
    ...
    def loss(self,
        ...
        if pos_idx.sum() > 0:
            ...
            ious = self.iou(pos_pred,pos_target).clone().detach()
            losses_cls, loss_monitoring, rank, order = self.ranking_loss(flat_preds, flat_labels, losses_bbox, ious)
            # try:
            #     losses_cls, order = self.fap_loss(flat_preds, flat_labels, loss_bbox, ious)
            ...
            #     else:
            #         raise e
            # loss_monitoring, rank,


            # aLRP Regression Component
            # rank = torch.arange(1,ordered_losses_bbox.numel()+1,1).cuda()
            # temp = (torch.cumsum(ordered_losses_bbox,dim=0)/rank)
            # losses_bbox = (temp.mean())
            # losses_bbox = loss_bbox.mean()*2
            ...
            # Self-balancing
            # self.cls_LRP_hist.append(float(loss_monitoring.item()))
